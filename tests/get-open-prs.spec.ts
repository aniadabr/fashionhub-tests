...
while (pageSize === 100) {
    const response = await request.get(`https://api.github.com/repos/appwrite/appwrite/pulls?state=open&per_page=100&page=${pageNumber}`, {
...
for (const pr of allPrs) {
  expect.soft(pr.title, `PR without title: ${JSON.stringify(pr)}`).toBeTruthy();
